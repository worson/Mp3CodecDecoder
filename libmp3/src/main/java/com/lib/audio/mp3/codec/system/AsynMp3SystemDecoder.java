 ...
 */
public class AsynMp3SystemDecoder {

    private final static String TAG = "Mp3Decoder";

    private Mp3DecodeListener mCodecListener;
    private AtomicBoolean mDecoding=new AtomicBoolean(false);
    private String mFilePath;

    public AsynMp3SystemDecoder(String filePath) {
        mFilePath = filePath;
    }

    public int start(){
        if (mFilePath == null || mCodecListener==null) {
            Log.e(TAG, "start: paras is error");
            return -1;
        }
        if (mDecoding.get()) {
            Log.i(TAG, "start: busy ....");
            return -1;
        }
    ...
    public void setCodecListener(Mp3DecodeListener codecListener) {
     ...
     * 解码并播放音频
     */
    private void decodeFile(final String filePath,final Mp3DecodeListener callback) {
        if (callback == null) {
            return;
        }
